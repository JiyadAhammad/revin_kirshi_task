...
Future<void> registerMainScreenDependencies() async {
  ...
  sl.registerLazySingleton<ResetResultUseCase>(
    () => ResetResultUseCase(sl<ISimulateResultRepository>()),
  );

  /// Bloc
  sl.registerFactory<SimulateBloc>(
    () => SimulateBloc(
      simulateResultUseCase: sl<SimulateResultUseCase>(),
      resetResultUseCase: sl<ResetResultUseCase>(),
    ),
  );
}
